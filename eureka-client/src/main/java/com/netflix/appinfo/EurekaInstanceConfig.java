...
public interface EurekaInstanceConfig {
     ...
     * .
     * 指示eureka服务器从接收到最后一次心跳开始等待的时间(以秒为单位)，然后通过不允许流量到此实例将此实例从其视图中删除。
     *
     * 将此值设置得太长可能意味着即使实例不活跃，也可以将流量路由到该实例。
     * 将此值设置得太小可能意味着，由于临时网络故障，实例可能会退出流量。
     *
     * 该值至少要高于getLeaseRenewalIntervalInSeconds()中指定的值。
     *
     *
     * 租约过期时间，单位：秒
     *
